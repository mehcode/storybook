...
const packageDir = path$2.resolve(node_url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.src || new URL('preset.cjs', document.baseURI).href))), "../..");
const distDir = path$2.resolve(node_url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.src || new URL('preset.cjs', document.baseURI).href))), "../..", "dist");
...
async function extendComposables(nuxt2) {
  ...
  addImportsSources({
    imports: ["useRouter"],
    from: path$2.join(composablesDir, "router")
  });
}
...
const viteFinal = async (config, options) => {
  const getStorybookViteConfig = async (c, o) => {
    const presetURL = node_url.pathToFileURL(
      path$2.join(await getPackageDir("@storybook/vue3-vite"), "preset.js")
    );
...
async function getPackageDir(frameworkPackageName) {
  try {
    ...
    const packageDir2 = path$2.dirname(
      require$1.resolve(path$2.join(frameworkPackageName, "package.json"), {
        paths: [process.cwd()]
      })
    );
    return packageDir2;
